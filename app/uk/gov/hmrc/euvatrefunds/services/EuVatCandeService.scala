...
class EuVatCandeService @Inject() (
  ...
  def getPurchaseDetails(request: GetPurchaseDetailsRequest)(implicit hc: HeaderCarrier): Future[GetPurchaseDetailsResponse] = {
    if (candeStubbed) {
      euVatStubsConnector.getPurchaseDetails(request)
    } else {
      rdsCandeProxyConnector.getPurchaseDetails(request)
    }
  }
  ...
  def getSupplierVrnCount(request: SupplierVrnCountRequest)(implicit hc: HeaderCarrier): Future[SupplierVrnCountResponse] = {
    if (candeStubbed) {
      euVatStubsConnector.getSupplierVrnCount(request)
    } else {
      rdsCandeProxyConnector.getSupplierVrnCount(request)
    }
  }
