...
import uk.gov.hmrc.euvatrefunds.services.EuVatCacheService
import uk.gov.hmrc.http.{HeaderCarrier, SessionId}

import scala.concurrent.{ExecutionContext, Future}

class EuVatCacheControllerSpec extends AnyWordSpec with Matchers with ScalaFutures with MockitoSugar {

  implicit val ec: ExecutionContext = ExecutionContext.global
  implicit val hc: HeaderCarrier = HeaderCarrier()

  private val service = mock[EuVatCacheService]
  ...
  private val controller =
    new EuVatCacheController(authAction, service, stubControllerComponents())
  ...
  private def callEndpoint() =
    controller.getTraderKnownFacts()(FakeRequest(GET, "/traders/getTraderKnownFacts"))

  "EuVatCacheController.getKnownFacts" should {

    "return 200 with JSON when service returns known facts" in {
      when(service.retrieveTraderKnownFacts(any())(any()))
